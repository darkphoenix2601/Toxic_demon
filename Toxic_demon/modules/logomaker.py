...
from Toxic_demon import OWNER_ID
...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./Toxic_demon/resources/1e0e5d446028d6e77385b4dd5d6e2254.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Toxic_demon/resources/Chopsic.otf", 100)
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @Toxic_demon_bot, Join @phoenix_music_suport")
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./Toxic_demon/resources/7ff02927132724dfc95db65bce4df3ce.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Toxic_demon/resources/Maghrib.ttf", 200)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
